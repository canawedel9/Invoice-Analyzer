...
2. Webhook: Go to Lovable app build the web app, copy the Production Webhook URL, set to 'Post', check on respond webhook nodes, make it receive chart data according to it, send action: submit (for submitted image or manual input) and action: analyze (for cart data)
